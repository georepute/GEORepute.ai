    ...
    switch (action) {
      case "approve":
        ...
        if (newStatus === "published") {
              ...
              if (shopifyIntegration && shopifyIntegration.access_token) {
                ...
                if (shopDomain) {
                  ...
                  shopifyResult = await publishToShopify(shopifyConfig, {
                    title: contentStrategy.topic || "Untitled",
                    body_html: wrapHtmlForRtl(contentStrategy.generated_content || "", contentStrategy.metadata?.contentLanguage),
                    author: "GeoRepute.ai",
          ...
          const contentLanguage = contentStrategy.metadata?.contentLanguage;
          const rawBodyHtml = contentStrategy.generated_content || "";
          const bodyHtmlRtl = wrapHtmlForRtl(rawBodyHtml, contentLanguage);
          let wordpressContent = bodyHtmlRtl;
          if ((platform === "wordpress" || platform === "wordpress_self_hosted") && contentStrategy.metadata?.useWordPressArticleTemplate) {
            ...
            const bodyLen = rawBodyHtml.length;
          ...
          if (platform === "wordpress_self_hosted") {
            ...
            try {
                  ...
                  wordpressSelfHostedResult = await publishToSelfHostedWordPress(
                    {
                      siteUrl,
                      username,
                      applicationPassword: wpSelfHostedIntegration.access_token,
                    },
                    {
                      title: contentStrategy.topic || "Untitled",
                      content: wordpressContent,
              ...
              console.error("Self-hosted WordPress publish error:", wpSelfHostedError);
          ...
          const insertData: any = {
            ...
            metadata: {
              ...
              wordpress_self_hosted: wordpressSelfHostedResult ? {
                success: wordpressSelfHostedResult.success,
                url: wordpressSelfHostedResult.url,
                postId: wordpressSelfHostedResult.postId,
                error: wordpressSelfHostedResult.error,
