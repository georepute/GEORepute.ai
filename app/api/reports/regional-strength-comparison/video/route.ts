...
const REPORT_TYPE_AI = "regional-strength-comparison-ai";
const REPORT_TYPE_GOOGLE = "regional-strength-comparison-google";
...
function buildVideoPrompt(report: ReportData, languageCode: string): string {
  ...
  const title = "Regional Strength Comparison";
  const tagline = "Identify weak regions and prioritize improvement efforts.";
...
[0-2s] TITLE: "${title} — ${q(domain, 30)}". Navy→purple gradient. Globe icon. Tagline: "${tagline}" Footer: GEORepute.ai ${formattedDate}.
...
function buildVideoPromptGoogle(report: ReportDataGoogle, languageCode: string): string {
  ...
  const langInstruction = useInstruction ? getLangInstruction(languageCode) : "";
  const title = "Region vs Google Search";
  const tagline = "Google Search Console performance by region.";
...
[0-2s] TITLE: "${title} — ${q(domain, 30)}". Navy→blue gradient. Globe icon. Tagline: "${tagline}" Footer: GEORepute.ai ${formattedDate}.
...
async function downloadAndStoreVideo(xaiVideoUrl: string, userId: string, domainId: string, reportType: string): Promise<string | null> {
  try {
    ...
    if (error) { console.error("[rsc-video] Upload error:", error); return null; }
    const { data: urlData } = supabaseAdmin.storage.from(VIDEO_BUCKET).getPublicUrl(fileName);
    return urlData.publicUrl;
  } catch (err) {
    console.error("[rsc-video] downloadAndStoreVideo error:", err);
    return null;
  }
}
  ...
  try {
    ...
    if (record.video_request_id && record.video_status === "pending") {
      ...
      try {
        ...
        console.error("[rsc-video] Poll error:", pollErr);
      }
    ...
    console.error("[rsc-video] GET error:", err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[rsc-video] Starting video generation for domain:", (reportData || reportDataGoogle)?.domain, "tab:", tabVariant, "language:", LANGUAGE_NAMES[languageCode?.toLowerCase().split("-")[0] || "en"] || "English");
    ...
    if (dbError) {
      console.error("[rsc-video] Failed to create/update report_videos record:", dbError);
    ...
    console.error("[rsc-video] POST error:", err);
