...
export default function MarketShareOfAttentionPage() {
  ...
  useEffect(() => {
    const projectId = selectedDomain?.projectId;
    if (video.status === "pending" && projectId) { startPolling(); } else { stopPolling(); }
    return () => stopPolling();
  }, [video.status, selectedDomain?.projectId]);
  ...
  const generateVideoReport = async () => {
    if (!report || !selectedDomain?.projectId) return;
    try {
      setGeneratingVideo(true);
      toast.loading("Starting video report generation with xAI Aurora...", { id: "gen-msa-video" });
      const reportData = { domain: report.domain, brandName: report.brand_name, aiMentionSharePct: report.ai_mention_share_pct, weightedAiSharePct: report.weighted_ai_share_pct, organicSharePct: report.organic_share_pct, marketShareScore: report.market_share_score, isDefaultLeader: report.is_default_leader, totalAiQueries: report.total_ai_queries, totalAiMentions: report.total_ai_mentions, totalRecommendations: report.total_recommendations, engineBreakdown: report.engine_breakdown, generatedAt: report.generated_at };
      const languageToUse = videoLanguageRef.current;
      const res = await fetch("/api/reports/market-share-of-attention/video", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId: selectedDomain.projectId, reportData, language: languageToUse }) });
      const data = await res.json();
      if (data.success && data.requestId) {
        setVideo({ status: "pending", url: null, generatedAt: null, requestId: data.requestId });
        toast.success("Video generation started! A 15-second video usually takes 2–8 minutes. We'll check automatically.", { id: "gen-msa-video", duration: 6000 });
      } else { toast.error(data.error || "Failed to start video generation", { id: "gen-msa-video" }); }
    } catch (err) { console.error("Generate video error:", err); toast.error("Failed to start video generation", { id: "gen-msa-video" }); }
  ...
  const pollVideoStatus = async () => {
    ...
    try {
      const res = await fetch(`/api/reports/market-share-of-attention/video?projectId=${projectId}`);
      const data = await res.json();
  ...
  const downloadVideo = async () => {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = blobUrl; a.download = `market-share-video-${report?.domain || "report"}-${Date.now()}.mp4`;
  ...
  useEffect(() => {
    ...
    if (!projectId) return;
    fetch(`/api/reports/market-share-of-attention/video?projectId=${projectId}`)
      .then(r => r.json())
      .then(data => {
          ...
          <div className="mb-6">
            {video.status === "idle" && (
              <div className="bg-gradient-to-r from-violet-50 to-indigo-50 rounded-xl border border-violet-200 p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex flex-shrink-0 items-center gap-2">
                  <label htmlFor="msa-video-lang-idle" className="text-sm font-medium text-gray-700 whitespace-nowrap">Language:</label>
                  <select id="msa-video-lang-idle" value={videoLanguage} onChange={(e) => { const v = e.target.value; setVideoLanguage(v); videoLanguageRef.current = v; }} disabled={generatingVideo} className="px-3 py-2.5 border border-violet-200 rounded-lg bg-white text-gray-700 text-sm font-medium disabled:opacity-50 min-w-[120px]" title="Video content language">{VIDEO_LANGUAGE_OPTIONS.map((opt) => <option key={opt.code} value={opt.code}>{opt.name}</option>)}</select>
                  <button onClick={generateVideoReport} disabled={generatingVideo || !selectedDomain?.projectId} className="px-5 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-lg hover:from-violet-700 hover:to-indigo-700 disabled:opacity-50 flex items-center gap-2 font-medium text-sm whitespace-nowrap"><Play className="w-4 h-4" />Generate Video Report</button>
                </div>
              </div>
            )}
            ...
            {video.status === "failed" && (
              <div className="bg-red-50 rounded-xl border border-red-200 p-6 flex items-center gap-4">
                ...
                <div className="flex flex-shrink-0 items-center gap-2">
                  <label htmlFor="msa-video-lang-failed" className="text-sm font-medium text-gray-700 whitespace-nowrap">Language:</label>
                  <select id="msa-video-lang-failed" value={videoLanguage} onChange={(e) => { const v = e.target.value; setVideoLanguage(v); videoLanguageRef.current = v; }} disabled={generatingVideo} className="px-3 py-2 border border-red-200 rounded-lg bg-white text-gray-700 text-sm font-medium disabled:opacity-50 min-w-[120px]" title="Video content language">{VIDEO_LANGUAGE_OPTIONS.map((opt) => <option key={opt.code} value={opt.code}>{opt.name}</option>)}</select>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between flex-wrap gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <label htmlFor="msa-video-lang-done" className="text-sm font-medium text-gray-600">Language:</label>
                    <select id="msa-video-lang-done" value={videoLanguage} onChange={(e) => { const v = e.target.value; setVideoLanguage(v); videoLanguageRef.current = v; }} disabled={generatingVideo} className="px-3 py-2 border border-gray-200 rounded-lg bg-white text-gray-700 text-sm font-medium disabled:opacity-50 min-w-[100px]" title="Video content language for regeneration">{VIDEO_LANGUAGE_OPTIONS.map((opt) => <option key={opt.code} value={opt.code}>{opt.name}</option>)}</select>
