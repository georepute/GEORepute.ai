  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const addHeader = () => {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(WHITE.r, WHITE.g, WHITE.b);
        doc.text("STRATEGIC BLIND SPOTS", pageWidth - margin, 9.5, { align: "right" });
        doc.setFontSize(6.5);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(PURPLE_LIGHT.r, PURPLE_LIGHT.g, PURPLE_LIGHT.b);
        doc.text(report!.domain, pageWidth - margin, 14, { align: "right" });
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        const scrollables = el.querySelectorAll<HTMLElement>("[class*='overflow-y-auto'], [class*='overflow-x-auto'], [class*='max-h-']");
        ...
        if (imgHeight > maxH) {
          ...
          while (remaining > 0) {
            ...
            if (ctx) {
              ...
              try {
                const sliceData = sc.toDataURL("image/jpeg", 0.92);
      ...
      const fileName = `Strategic_Blind_Spots_${report.domain.replace(/[^a-zA-Z0-9]/g, "_")}_${new Date().toISOString().split("T")[0]}.pdf`;
      doc.save(fileName);
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className={`grid grid-cols-1 ${report ? "md:grid-cols-4" : "md:grid-cols-3"} gap-4`}>
          <div>
          ...
          {report && (
            <div className="flex items-end">
              <button
                ...
                className="w-full px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 font-medium"
              >
        ...
        <div ref={reportRef}>
          ...
          {(() => {
            ...
            return llmNotMentioned.length > 0 ? (
              <div data-pdf-section className="mb-6 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl shadow-sm border border-orange-200 p-6">
          ...
          {perLlmBarData.length > 0 && (
            <div data-pdf-section className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
