...
async function getApiKey(keyType) {
  try {
    // Try multiple key naming conventions for backward compatibility
    const keyVariants = [
      keyType,
      `admin_${keyType}_key` // e.g., "admin_claude_key", "admin_gemini_key"
    ];
    // Try each variant in database
    for (const variant of keyVariants){
      const { data, error } = await supabase.from('admin_api_keys').select('encrypted_value, encrypted_key').eq('key_type', variant).eq('is_active', true).maybeSingle();
      if (data) {
        // Try encrypted_value first (newer format), then encrypted_key (legacy format)
        const encryptedKey = data.encrypted_value || data.encrypted_key;
        if (encryptedKey) {
          const decrypted = atob(encryptedKey);
          console.log(`✅ ${keyType} key loaded from database (${variant})`);
          return decrypted;
        }
      }
    }
    ...
    return Deno.env.get(`${keyType.toUpperCase()}_API_KEY`) || null;
  }
}
...
async function processBatchOfQueries(projectId, platforms, sessionId, queries, batchStartIndex = 0, batchSize = 3 // Reduced default batch size from 5 to 3
  ...
  try {
    ...
    if (hasMoreQueries && newCompletedQueries < totalQueries) {
      ...
      fetchWithRetry(`${Deno.env.get('SUPABASE_URL')}/functions/v1/brand-analysis-processor`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${Deno.env.get('SUPABASE_ANON_KEY')}`,
          'Content-Type': 'application/json'
