  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data.message && data.message.includes('background')) {
        toast.error(
          `Analysis Started! Processing ${data.total_queries} queries across ${data.platforms_analyzed.length} platforms in background. Check back in a few minutes.`
        );
      } else {
        toast.success("Brand visibility analysis is running. Results will appear shortly.");
      }
    },
    onError: (error, variables) => {
      ...
      toast.error("Failed to start brand visibility analysis. Please try again.");
